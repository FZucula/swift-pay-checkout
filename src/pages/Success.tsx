...
const Success = () => {
    ...
    useEffect(() => {
        ...
        // Recuperar o successIndicator do localStorage
        const storedSuccessIndicator = localStorage.getItem("successIndicator");
        ...
        console.log(storedSuccessIndicator);

        // Se o resultIndicator da URL for igual ao successIndicator do localStorage, então houve pagamento, se não, não houve pagamento
        if (resultIndicator !== storedSuccessIndicator) {
            ...
            const paymentData = {
                paymentId: resultIndicator || `mpesa_${Date.now()}`,
                amount: 6950,
                method: "cartao",
                status: "success",
                phoneNumber: '0000000000',
                details: 'result',
                name: localStorage.getItem("purchaserName"),
                email: localStorage.getItem("purchaserEmail"),
            };

            // Post name, email and phoneNumber to backend
            fetch("https://soico.app.n8n.cloud/webhook/webhook/payment-success", {
                method: "POST",
                headers: {
                    ...
                    <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-lg">
                        <div className="p-8 md:p-12 text-center space-y-6">
                            ...
                            <div className="space-y-3">
                                <h1
                                    className={`text-3xl md:text-4xl font-bold ${status === "success"
                                        ? "text-green-500"
                                        : status === "pending"
                                            ? "text-amber-500"
                                            : status === "error"
                            ...
                            <div className="bg-muted/30 rounded-lg p-4 text-sm text-muted-foreground border border-border/50">
                                ...
                                <p>
                                    <span className="font-semibold">Versão da Sessão:</span>{" "}
                                    <span className="text-foreground">
                                        {searchParams.get("sessionVersion") || "N/A"}
                                    </span>
                                </p>
